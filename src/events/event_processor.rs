...
pub async fn process_evm_events(
    ...
    while let Some(log) = stream.next().await {
        // Since mockLN contract calls vault, we need to ensure that we dont process multiple events for the same order
        if is_mockln {
            match log.topic0() {
            ...
            match log.topic0() {
                ...
                Some(&IntentProcessorV2::AcknowledgementReceived::SIGNATURE_HASH) => {
                    ...
                    let decoded_ack_metadata =
                        SolidityAcknowledgementMetadata::abi_decode(metadata.as_ref(), true)
                            .unwrap();
